...
def main(TopK:str,threshold:str):
    ...
    for thd in np.arange(float(threshold), -0.1, -0.1):
        ...
        trianglesFoundAlready = sc.sparkContext.broadcast(dict( [(row[0],row[1]) for row in TopKTrianglesInHeavySet] ))
